...
namespace CapaPresentacion.Reports {
    public class PRU_AlertaReport {
        public static MemoryStream ExcelAlertas(List<PRU_AlertaDto> items) {
            MemoryStream memoryStream = new MemoryStream();

            ExcelPackage.LicenseContext = LicenseContext.NonCommercial;

            using(ExcelPackage excelPackage = new ExcelPackage()) {
                ExcelWorksheet worksheet = excelPackage.Workbook.Worksheets.Add("Alertas");

                WorksheetAlertas(worksheet, items);
        ...
        private static ExcelWorksheet WorksheetAlertas(ExcelWorksheet worksheet, List<PRU_AlertaDto> items) {
            // parameters
            ...
            worksheet.Cells[2, 2, 3, 8].Style.HorizontalAlignment = ExcelHorizontalAlignment.Center;
            worksheet.Cells[2, 2, 3, 8].Style.VerticalAlignment = ExcelVerticalAlignment.Center;
            ...
            worksheet.Cells[2, 2].Value = $"Alertas Progresivo Ruleta";
            ...
            worksheet.Cells[fromRow, 5].Value = "Cod. Máquina";
            worksheet.Cells[fromRow, 6].Value = "Monto";
            worksheet.Cells[fromRow, 7].Value = "Detalle";
            worksheet.Cells[fromRow, 8].Value = "Fecha y Hora";

            // head style
            worksheet.Row(fromRow).Height = rowHeight + 7;
            ...
            foreach(var item in items) {
                ...
                worksheet.Cells[recordRow, 7].Value = item.Detalle;

                if(nullDates.Contains(item.FechaAlerta.ToString("dd-MM-yyyy"))) {
                    worksheet.Cells[recordRow, 8].Value = "";
                } else {
                    worksheet.Cells[recordRow, 8].Value = item.FechaAlerta.ToString("dd/MM/yyyy HH:mm:ss");
                }
