...
btnSendMessage.on("click", (e) => {
    ...
    if (!phones) {
        toastr.info("Seleccione al menos un destinatario.")
        return;
    }

    var aux = phones.length == 1 ? 'destinatario' : 'destinatarios';

    $.confirm({
        title: 'Confirmación',
        content: `¿Seguro que desea enviar el mensaje a ${phones.length} ${aux}?`,
        confirmButton: 'Sí',
        cancelButton: 'No',
        confirm: function () {
            sendMultipleMessage(phones, message);
        },
        cancel: function () {
...
const sendMultipleMessage = (ids, message) => {
    let url = `${basePath}Mensajeria/EnviarMensajeMultiple`;
...
const getActiveContacts = () => { 
    var url = basePath + "Contacto/ListarLosContactosActivosJSON";
    var data = {};
        ...
        success: function (response) {
            if (response.success) {
                ...
                $.each(data, function (index, value) {
                    $("#cboContactos").append('<option value="' + value.IdContacto + '"  >' + value.Nombre + '</option>');
                });
                $("#cboContactos").select2({
                    placeholder: "--Seleccione--", allowClear: true, multiple: true,
                });
                $("#cboContactos").val(null).trigger("change");
                toastr.success(response.displayMessage);
            } else {
                toastr.error(response.displayMessage);
            }
