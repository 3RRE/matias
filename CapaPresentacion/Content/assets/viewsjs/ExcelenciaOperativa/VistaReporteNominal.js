...
function getNominalReport(params) {
    ...
    $.ajax({
        type: "POST",
        url: `${basePath}ExcelenciaOperativa/ListarReporteNominal`,
...
function tableNominalReport(type, items, data)
{
    ...
    items.map((category, indexA) => {
        ...
        data.map(dataItem => {
            $(`#categoriesWrap${indexA}`).append(`<th ficha-id="${dataItem.FichaId}">Respuesta</th>`)
        })
    ...
    data.map(dataItem => {
        ...
        dataItem.Categorias.map((category, indexA) => {
            category.Items.map((item, indexB) => {
                if (item.Codigo)
                {
                    tbodyWrap.find(`[item-code="${item.Codigo}"] [ficha-id="${dataItem.FichaId}"]`).html(item.Respuesta)
                }
                //si el valor del item es de tipo 7 ==comentario entonces mostrara un boton, solo si la observacion tiene algun valor
                /********************************************************************************************************************* */
                ...
                else
                {
                    ...
                    if (type == 1 && indexA == 0)
                    {
                        if (indexB <= 5)
                        {
                            tbodyWrap.find(`[ficha-id="${dataItem.FichaId}"][indexa="${indexA}"][indexb="${indexB}"]`).html(item.Respuesta)
                        }
                        else
                        {
                            tbodyWrap.find(`[ficha-id="${dataItem.FichaId}"][indexa="${indexA}"][indexb="${indexB + 1}"]`).html(item.Respuesta)
                        }
                    }
                    else
                    {
...
function downloadExcelNominal(params)
{
    ...
    $.ajax({
        type: "POST",
        url: `${basePath}ExcelenciaOperativa/DescargarExcelNominal`,
        data: JSON.stringify({ ...data }),
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        processdata: true,
