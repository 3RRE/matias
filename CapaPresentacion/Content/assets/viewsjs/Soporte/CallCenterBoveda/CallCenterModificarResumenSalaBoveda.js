$(document).ready(function () {
    var Proceso_Eje = "CallCenterCajaBoveda/CallCenterModificarResumenSalaBovedaVista";
    ...
    $('.btnListar').on('click', function (e) {
        window.location.replace(basePath + "CallCenterCajaBoveda/CallCenterResumenSalaBovedaVista");
    });

    $('.btnGuardar').on('click', function (e) {
        auditoriaGuardar_IAS(Proceso_Eje, "Guardar Resumen Boveda");
        ...
        if (validar.isValid()) {
            $.confirm({
                ...
                confirm: function () {
                    ...
                    var url = $("#ip_resumen").val() + "/servicio/ModificarResumenSalaBoveda?fechaOperacion=" + $("#fechaOperacion").val() +
                    ...
                    $.when(dataAuditoria(1, "#frmRegistroResumenSala", 3, "CallCenterCajaBoveda/CallCenterModificarResumenSalaBovedaVista", "ACTUALIZAR RESUMEN SALA")).then(function (response, textStatus) {
    ...
    $.when(VistaAuditoria("CallCenterCajaBoveda/CallCenterModificarResumenSalaBovedaVista", "VISTA", 1, "#frmRegistroResumenSala", 3)).then(function (response, textStatus) {
        dataAuditoria(0, "#frmRegistroResumenSala", 3);
    });
});

function DataResumenSala() {
    var NombreUsuarioLogeado = $("#txtNombreUsuarioLogeado").val();
    if (!$("#ip_resumen").val().trim()) {
        toastr.error("Sala sin URL Asignada", "Mensaje Servidor");
        return false;
    }
    var url = $("#ip_resumen").val() + "/servicio/ObtenerResumenSalaIdBoveda?codResumen=" + $("#id_resumen").val() + "&usuarioId=" + NombreUsuarioLogeado;
    $.ajax({
        type: "POST",
        cache: false,
