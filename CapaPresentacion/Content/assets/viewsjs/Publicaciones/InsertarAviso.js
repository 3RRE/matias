$(document).ready(function () {
    ...
    // Validadores
    $("#frmRegistroPublicidad").bootstrapValidator({
        container: '#messages',
    ...
    $(document).on('click', '.btnGuardar', function (e) {
        ...
        let validar = $("#frmRegistroPublicidad").data('bootstrapValidator').validate();
        if (validar.isValid()) {

            let file = $('#RutaImagen')[0].files[0];
            if (!file) {
                toastr.warning("Seleccione una imagen para la publicidad", "Mensaje Servidor");
                return;
            }

            // Validar tamaño (ej. 5MB)
            let permitido = 5242880; // 5 * 1024 * 1024
            if (file.size > permitido) {
                toastr.error("La imagen no debe pesar más de 5MB", "Mensaje Servidor");
                return;
            }

            let dataForm = new FormData(document.getElementById("frmRegistroPublicidad"));

            $.ajax({
                url: basePath + "Publicidad/InsertarPublicidadJson",
                type: "POST",
                method: "POST",
                contentType: false,
                data: dataForm,
                ...
                success: function (response) {
                    if (response.respuesta) {
                        ...
                        $.confirm({
                            ...
                            cancel: function () {
                                $("#frmRegistroPublicidad").data('bootstrapValidator').resetForm(true);
                            }
                        });
                    } else {
