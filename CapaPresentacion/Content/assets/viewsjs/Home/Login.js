...
$(document).ready(function () {
            ...
            DataPostSend(url, data, true).done(function (response) {
                if (response) {
                    ...
                    if (response.CambioContrasena) {
                        setTimeout(function () {
                            window.location.replace(basePath + "Home/CambioContrasena");
                        }, 500);
                    }
                    else {
                        if (data) {
                            toastr.success(response.mensaje, "Mensaje Servidor");
                            var redirectUrl = basePath + "Home/DashBoard";
    ...
    function compruebaDispositivo() {
        var activo = 0;
        $.ajax({
            async: false,
            url: basePath + '/dispositivo/ComprobarDispositivoJson',
            data: {mac : code},
            type: 'POST',
            dataType: 'json',
