VistaAuditoria("UsuarioSala/UsuarioSalaVista", "VISTA", 0, "", 3);
$(document).ready(function () {
    ...
    //llenarSelect(basePath + '/Usuario/UsuarioListadoJson', "", "cboUsuario", "UsuarioID", "UsuarioNombre", "");
    //$("#cboUsuario").select2();
        ...
        if (idUsuario != '') { 
            ...
            $.ajax({
                ...
                success: function (response) { 
                        ...
                        if (data == 0) {
                            $.ajax({
                                url: basePath + '/UsuarioSala/UsuarioSalaInsertarJson',
                                type: "POST",
                                contentType: "application/json",
                                ...
                                success: function (response) { 
                                    if (response.respuesta == true) {
                                        ...
                                        dataAuditoriaJSON(3, "UsuarioSala/UsuarioSalaInsertarJson", "AGREGAR SALA", datainicial, datafinal);
            ...
            $.ajax({
                ...
                success: function (response) { 
                    ...
                    if (data == 0) {
                        ...
                        $.ajax({
                            url: basePath + '/UsuarioSala/UsuarioSalaEliminarJson',
                            type: "POST",
                            contentType: "application/json",
                            data: JSON.stringify({ usuarioId: idUsuario, salaId: idSala }),
                            success: function (response) { 
                                if (response.respuesta == true) {
                                    ...
                                    dataAuditoriaJSON(3, "UsuarioSala/UsuarioSalaEliminarJson", "QUITAR SALA", datainicial, datafinal);
...
function getUsers()
{
    ...
    $.ajax({
        url: `${basePath}Usuario/UsuarioListadoJson`,
        type: "POST",
        contentType: "application/json",
        cache: false,
        beforeSend: function () {
        ...
        success: function (response) {
            ...
            items.map(item => {
                element.append(`<option value="${item.UsuarioID}" data-name="${item.UsuarioNombre}">${item.UsuarioNombre}</option>`)
            })
        },
        complete: function () {
            element.select2()
    ...
    var AppRoomUser = new Vue({
        ...
        computed: {
            ...
            filteredRooms: function () {
                return this.roomes.filter(room => {
                    return room.name.toLowerCase().includes(this.roomSearch.toLowerCase()) || room.company.name.toLowerCase().includes(this.roomSearch.toLowerCase())
                })
            },
            getCompany: function () {
        ...
        methods: {
            getDataInitial: function () {
                ...
                $.ajax({
                    url: `${basePath}UsuarioSala/ObtenerEmpresaSala`,
                    type: "POST",
                    contentType: "application/json",
                    cache: false,
                    beforeSend: function () {
            ...
            setDataInitial: function (response) {
                ...
                rooms.map(item => {
                    self.rooms.push({
                        code: item.CodSala,
                        name: item.Nombre,
                        company: self.companies.find(company => company.code == item.CodEmpresa),
                        checked: false
                    })
                })

                self.companies.map(item => {
                    item.rooms = self.rooms.filter(room => room.company.code == item.code)
                })
            ...
            setRoomChecked: function (room, checked) {
                var roomIndex = this.rooms.findIndex(item => item.code == room)
            ...
            setUserRoomChecked: function (items) {
                ...
                self.rooms.forEach((room, key) => {
                    ...
                    if (items.some(item => item.SalaId == room.code && item.UsuarioId == self.user.id)) {
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (status) {
                            ...
                            if (toaster == 'CKL_01') {
                                toastr.success(`Se asignaron ${dataLength} salas al usuario ${userName}`, "Mensaje Servidor")
                            }

                            if (toaster == 'CKL_02') {
                                toastr.success(`Se asignó una sala al usuario ${userName}`, "Mensaje Servidor")
                            }

                            if (toaster == 'UKL_01') {
                                toastr.warning(`Se denegaron ${dataLength} salas al usuario ${userName}`, "Mensaje Servidor")
                            }

                            if (toaster == 'UKL_02') {
                                toastr.warning(`Se denegó una sala al usuario ${userName}`, "Mensaje Servidor")
                            }
                            ...
                            var descriptionProcess = checked ? 'AGREGAR SALA' : 'QUITAR SALA'
            ...
            icheck: {
                inserted: function (el, binding, vnode) {
                    ...
                    $(el).on('ifChecked', function (event) {
                        ...
                        var url = 'UsuarioSala/UsuarioSalaInsertarJson'
                        ...
                        if (layer == 1)
                        {
                            var messageConfirm = `¿Estás seguro de asignar todas las salas para el usuario <b>${context.user.name}</b>?`

                            if (context.companySelected != -1 && !context.isViewFull) {
                                messageConfirm = `¿Estás seguro de asignar todas las salas de ${context.getCompany.name} para el usuario <b>${context.user.name}</b>?`
                            }
                    ...
                    $(el).on('ifUnchecked', function (event) {
                        ...
                        var assignments = []
                        var assigRooms = context.rooms.filter(item => item.checked)
                        var url = 'UsuarioSala/UsuarioSalaEliminarJson'
                        ...
                        if (layer == 1)
                        {
                            var messageConfirm = `¿Estás seguro denegar todas las salas asignadas para el usuario <b>${context.user.name}</b>?`

                            if (context.companySelected != -1 && !context.isViewFull) {
                                messageConfirm = `¿Estás seguro denegar todas las salas asignadas de ${context.getCompany.name} para el usuario <b>${context.user.name}</b>?`
                            }
