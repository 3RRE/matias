...
const cboEmpresaEquivalencia = $("#cboEquivalenciaEmpresa");
const btnGuardarModal = $("#btnGuardarModal");

const guardarEmpresasEquivalencia = (data) => {
    let url = basePath + "EquivalenciaSede/InsertarEquivalenciaSedeJSON";
    $.ajax({
        url: url,
        type: "POST",
...
const renderSelectEmpresasEquivalencias = (data) => {
    $.each(data, function (index, value) {
        cboEmpresaEquivalencia.append(`<option value="${value.IdEquivalenciaEmpresa}">${value.Nombre}</option>`)
    });
    cboEmpresaEquivalencia.select2({
        placeholder: "--Seleccione--", allowClear: true, minimumResultsForSearch: 5, dropdownParent: $('#modalGroup')
    });
    cboEmpresaEquivalencia.val(null).trigger("change");
}
...
$(document).ready(function () {
    ...
    btnGuardarModal.on('click', function (e) {
        ...
        guardarEmpresasEquivalencia(dataForm);
    });
});
