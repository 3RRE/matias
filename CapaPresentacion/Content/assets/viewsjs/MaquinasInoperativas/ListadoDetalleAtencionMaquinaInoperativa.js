...
$(document).ready(function () {
    ...
    if (maquinaInoperativa.Estado == 4) {
        $(".btnAtenderIgual").attr('style', 'display:block');
    }


    $(document).on("click", ".btnAtenderIgual", function () {
    ...
    $(document).on("click", ".btnListar", function () {
        ...
        window.location.replace(basePath + "MIMaquinaInoperativa/ListadoMaquinaInoperativaAtendida");
...
*/
    ...
    $(document).on("click", ".btnDescontarRepuesto", function () {
        ...
        var data = { CodRepuesto: codRepuesto, CodPiezaRepuestoAlmacen: codAlmacen, Repuesto: repuesto, Cantidad: cantidad, Estado: estado, CodAlmacenOrigen: 0, CodAlmacenDestino: 0 };
...
function EnviarRepuesto() {
    ...
    var data = { CodRepuesto: CodRepuesto, CodPiezaRepuestoAlmacen: CodPiezaRepuestoAlmacen, Repuesto: NombreRepuesto, Cantidad: Cantidad, Estado: Estado, CodAlmacenOrigen: CodAlmacenOrigen, CodAlmacenDestino: CodAlmacenDestino };
        ...
        success: function (response) {
            ...
            if (response.respuesta) {
                ...
                const arrayAlmacenPropio = response.data.map(propio => {
                    return { ...propio, tipo: 1 }
                })
                const arrayAlmacenAyuda = response.data2.map(propio => {
                    return { ...propio, tipo: 2 }
                })

                const nuevoArrayALmacen = arrayAlmacenPropio.concat(arrayAlmacenAyuda)
                console.log("aca estaaaa")
                console.log(nuevoArrayALmacen)
