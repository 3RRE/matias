...
$(document).ready(function () {
    ...
    $(document).on("click", ".btnListar", function () {
        ...
        window.location.replace(basePath + "MIMaquinaInoperativa/ListadoMaquinaInoperativaAtendidaInoperativa");
    ...
    $(document).on("click", ".btnAgregarRepuestonuevalogica", function () {
        ...
        var data = { CodRepuesto: codRepuestonuevalogica, NombreAlmacen: ' - ', NombreAlmacenDestino: ' - ', CodPiezaRepuestoAlmacen: codRepuestonuevalogica, Repuesto: Repuestonuevalogica, Cantidad: cantidadRepuestonuevalogica, Estado: estado, CodAlmacenOrigen: 0, CodAlmacenDestino: 0, Enviar: true };
    ...
    $(document).on("click", ".btnPedirRepuesto", function () {
        ...
        codAlmacenGlobal = $(this).data("piezarepuestoalmacenid");
    ...
    $(document).on("click", ".btnElegirAlmacenDestino", function () {
        ...
        $("#full-modal_cantidad").modal("show");
    ...
    $(document).on("click", ".btnAgregarCantidad", function () {
        ...
        let NombreAlmacenDestino = $("#cboAlmacenDestino option:selected").text();
        ...
        var data = { CodRepuesto: CodRepuesto, NombreAlmacen: nombreAlmacenGlobal, NombreAlmacenDestino: NombreAlmacenDestino, CodPiezaRepuestoAlmacen: CodPiezaRepuestoAlmacen, Repuesto: NombreRepuesto, Cantidad: Cantidad, Estado: Estado, CodAlmacenOrigen: CodAlmacenOrigen, CodAlmacenDestino: CodAlmacenDestino, Enviar: true };
        ...
        $("#full-modal_cantidad").modal("hide");
    ...
    $(document).on("click", ".btnAtender", function () {
        ...
        if (arrayRepuestosPedidos.length==0) {
            toastr.error("Necesita agregar por los menos 1 repuesto.", "Mensaje Servidor");
            return false;
        }
...
function LoadAlmacenDestino() {
    ...
    $.ajax({
        type: "POST",
        url: basePath + "MIAlmacen/GetAllAlmacenUsuario",
        cache: false,
        contentType: "application/json; charset=utf-8",
        data: JSON.stringify({ cod: maquinaInoperativa.CodUsuarioAtendidaInoperativa }),
        dataType: "json",
        beforeSend: function (xhr) {
            $.LoadingOverlay("show");
        },
...
function DataTableRepuestosxAlmacen(cod) {
    ...
    $.ajax({
        type: "POST",
        cache: false,
        url: basePath + "MIMaquinaInoperativa/BuscarInventarioRepuestoAlmacenesAjenos",
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: JSON.stringify({ cod }),
        beforeSend: function (xhr) {
                        ...
                        },
                        {
                            ...
                            "render": function (o, type, oData) {
                                ...
                                if (oData.tipo == 'propio') {
                                    ...
                                    return `<button type="button" class="btn btn-xs btn-info btnPedirRepuesto" data-id="${oData.CodAlmacen}" data-salaid="${oData.CodSala}"  data-repuestoid="${oData.CodPiezaRepuesto}"  data-piezarepuestoalmacenid="${o}" data-nombrealmacen="${oData.NombreAlmacen}" data-cantidad="${cantidadActual}"><i class="glyphicon glyphicon-hand-right"></i></button>`;
                                }
                            }
                        }
                    ],
...
function enviarCorreoProcesoMaquinaInoperativa(CodSala) {
    ...
    $.ajax({
        ...
        contentType: "application/json; charset=utf-8",
        dataType: "json",
        data: JSON.stringify({ CodSala, CodTipo: 2 }),
        beforeSend: function (xhr) {
            $.LoadingOverlay("show");
        },
        success: function (result) {
