...
$(document).ready(function () {
    ...
    DataTableRepuestoAgregadoUpdate(arrayRepuestosAgregados);
    ...
    $("#ObservacionAnterior").val(maquinaInoperativa.ObservacionAtencion);
    $("#ObservacionAtencion").val("");
    $("#cboAtencion").val(2);
    $("#cboAtencion").change();
...
*/
    ...
    $(document).on("click", ".btnGuardar", function () {
        ...
        if (validar.isValid()) {
            ...
            let ObservacionAtencion = $("#ObservacionAnterior").val()+" "+ $("#ObservacionAtencion").val();
            ...
            $.ajax({
                ...
                dataType: "json",
                data: JSON.stringify({ CodMaquinaInoperativa, TipoAtencion, ObservacionAtencion, TecnicoAtencion, listaProblemas, listaPiezas, listaRepuestos, listaPiezasCantidad, listaRepuestosCantidad, listaRepuestosCodPiezaRepuestoAlmacen }),
    ...
    listaRepuestosAgregados.forEach(function (item, index) {

        var data = { CodRepuesto: item.CodRepuesto, CodPiezaRepuestoAlmacen: 0, Repuesto: item.NombreRepuesto, Cantidad: item.Cantidad, Estado: item.Estado, CodAlmacenOrigen: 0, CodAlmacenDestino: 0, Enviar: false };

        arrayRepuestosAgregados.push(data);

    });

    //console.log(arrayRepuestosAgregados);
...
function DataTableRepuestoAgregadoUpdate(arrayRepuesto) {


    objetodatatablePieza = $("#tableRepuestoAgregados").DataTable({
        "bDestroy": true,
        "ordering": false,
        "scrollCollapse": true,
        "scrollX": true,
