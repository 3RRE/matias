...
btnNuevo.on('click', function () {
    ...
    $.when(DataPostModo1(`${basePath}Marcas/Guardar`, 'bodyModal')).then(() => {
...
$(document).on("click", ".btnEditar", function () {
    ...
    $('#bodyModal').html("");
    $('#full-modal-label').html(nombre);
    $.when(DataPostModo1(`${basePath}Marcas/Guardar/${id}`, 'bodyModal')).then(() => {
        $("#full-modal").modal("show");
    });
});

const getMarcas = () => {
    const url = `${basePath}Marcas/GetMarcas`;
    $.ajax({
        url: url,
        type: "POST",
    ...
    objetodatatable = $("#table").DataTable({
        ...
        columns: [
            ...
            },
            {
                ...
                render: (value) => {
                    return `
                        <button type="button" class="btn btn-xs btn-warning btnEditar" data-id="${value}" data-nombre="Editar Marca"><i class="glyphicon glyphicon-pencil"></i></button>
...
const deleteMarca = (id) => {
    $.ajax({
        url: `${basePath}Marcas/DeleteMarca/${id}`,
        type: "POST",
        contentType: "application/json",
        beforeSend: () => $.LoadingOverlay("show"),
        complete: () => $.LoadingOverlay("hide"),
