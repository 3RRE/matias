...
btnNuevo.on('click', function () {
    ...
    $.when(DataPostModo1(`${basePath}SubTipos/Guardar`, 'bodyModal')).then(() => {
...
$(document).on("click", ".btnEditar", function () {
    ...
    $('#bodyModal').html("");
    $('#full-modal-label').html(nombre);
    $.when(DataPostModo1(`${basePath}SubTipos/Guardar/${id}`, 'bodyModal')).then(() => {
        $("#full-modal").modal("show");
    });
});

const getSubTipos = () => {
    const url = `${basePath}SubTipos/GetSubTipos`;
    $.ajax({
        url: url,
        type: "POST",
    ...
    objetodatatable = $("#table").DataTable({
        ...
        columns: [
            ...
            { data: "Nombre", title: "Nombre" },
            {
                ...
                , "render":
                    function (value) {
                        let myLink = ` <img src="${value}"  width="40" height="40" alt="SubTipo" />`
                        return myLink;
                    },
                "orderable": false,
                className: "text-center",
            ...
            },
            {
                ...
                render: (value) => {
                    return `
                        <button type="button" class="btn btn-xs btn-warning btnEditar" data-id="${value}" data-nombre="Editar SubTipo"><i class="glyphicon glyphicon-pencil"></i></button>
...
const deleteSubTipo = (id) => {
    $.ajax({
        url: `${basePath}SubTipos/DeleteSubTipo/${id}`,
        type: "POST",
        contentType: "application/json",
        beforeSend: () => $.LoadingOverlay("show"),
        complete: () => $.LoadingOverlay("hide"),
