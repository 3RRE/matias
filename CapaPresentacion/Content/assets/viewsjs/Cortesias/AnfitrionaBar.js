        ...
        if (idUsuario != '') {
            ...
            $.ajax({
                ...
                success: function (response) {
                    ...
                    if (data == 0) {
                        $.ajax({
                            url: basePath + '/AnfitrionaBar/AnfitrionaBarAsginar',
                            type: "POST",
                            contentType: "application/json",
                            ...
                            success: function (response) {
                                if (response.respuesta == true) {
                                    ...
                                    dataAuditoriaJSON(3, "AnfitrionaBar/AnfitrionaBarAsginar", "AGREGAR ANFITRIONA", datainicial, datafinal);
                        ...
                        $.ajax({
                            ...
                            success: function (response) {
                                if (response.respuesta == true) {
                                    ...
                                    dataAuditoriaJSON(3, "AnfitrionaBar/AnfitrionaBarAsginar", "QUITAR ANFITRIONAS", datainicial, datafinal);
...
function getUsers(codSala) {
    ...
    $.ajax({
        url: `${basePath}AnfitrionaBar/GetEmpleadosBar`,
        type: "POST",
    ...
    var AppRoomUser = new Vue({
        ...
        computed: {
            filteredRooms: function () {
                return this.roomes.filter(room => {
                    return room.name.toLowerCase().includes(this.roomSearch.toLowerCase()) || room.detail.toLowerCase().includes(this.roomSearch.toLowerCase())
                })
            },
        ...
        methods: {
            getDataInitial: function () {
                ...
                $.ajax({
                    url: `${basePath}AnfitrionaBar/GetAnfitrionas`,
                    type: "POST",
                    data: JSON.stringify({ codSala }),
                    contentType: "application/json",
                    cache: false,
            ...
            setUserRoomChecked: function (items) {
                ...
                self.rooms.forEach((room, key) => {
                    ...
                    if (items.some(item => item.IdUsuarioAnfitriona == room.code)) {
            ...
            getAssignmentsUserRoom: function (userId) {
                ...
                $.ajax({
                    url: `${basePath}AnfitrionaBar/GetAnfitrionasBar`,
                    type: "POST",
                ...
                $.ajax({
                    ...
                    success: function (response) {
                        ...
                        if (status) {
                            ...
                            if (toaster == 'CKL_01') {
                                toastr.success(`Se asignaron ${dataLength} anfitrionas al bar ${userName}`, "Mensaje Servidor")
                            }

                            if (toaster == 'CKL_02') {
                                toastr.success(`Se asignó una anfitriona al bar ${userName}`, "Mensaje Servidor")
                            }

                            if (toaster == 'UKL_01') {
                                toastr.warning(`Se denegaron ${dataLength} anfitrionas al bar ${userName}`, "Mensaje Servidor")
                            }

                            if (toaster == 'UKL_02') {
                                toastr.warning(`Se denegó una anfitriona al bar ${userName}`, "Mensaje Servidor")
                            }
                            ...
                            var descriptionProcess = checked ? 'AGREGAR ANFITRIONA' : 'QUITAR ANFITRIONA'
            ...
            icheck: {
                inserted: function (el, binding, vnode) {
                    ...
                    $(el).on('ifChecked', function (event) {
                        ...
                        var url = 'AnfitrionaBar/AnfitrionaBarAsginar'

                        toastr.clear()

                        if (!context.user.id) {
                            toastr.warning('Seleccione un Bar', 'Mensaje Servidor')

                            return false
                        }

                        if (layer == 1) {
                            var messageConfirm = `¿Estás seguro de asignar todas las anfitrionas para el bar <b>${context.user.name}</b>?`
                    ...
                    $(el).on('ifUnchecked', function (event) {
                        ...
                        if (layer == 1) {
                            var messageConfirm = `¿Estás seguro denegar todas las anfitrionas para el bar <b>${context.user.name}</b>?`
