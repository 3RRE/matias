$(document).ready(function () {
    ...
    llenarSelect(basePath + "GestionCambios/CargarListaSistemasJson", {}, "cboSistema", "SistemaId", "Descripcion", modulo.SistemaID);
    $("#txtModuloId").val(modulo.ModuloId);
    $("#txt_descripcion").val(modulo.Descripcion);
    $("#cboEstado").val(modulo.Estado);
    ...
    $(".btnGuardar").on('click', function () {
        ...
        if (validar.isValid()) {
            var url = basePath + "GestionCambios/ModuloActualizarJson";
            var redirectUrl = basePath + "GestionCambios/GestionCambiosListadoModuloVista";
            var dataForm = $('#frmNuevoModulo').serializeFormJSON();
            $.when(dataAuditoria(1, "#frmNuevoModulo", 3, "GestionCambios/ModuloActualizarJson", "EDITAR MODULO")).then(function (response, textStatus) {
                enviarDataPost(url, dataForm, false, false, redirectUrl, true);
            });
        }
    });

    $.when(VistaAuditoria("GestionCambios/GestionCambiosEditarModuloVista", "VISTA", 1, "#frmNuevoModulo", 3)).then(function (response, textStatus) {
        dataAuditoria(0, "#frmNuevoModulo", 3);
    });
