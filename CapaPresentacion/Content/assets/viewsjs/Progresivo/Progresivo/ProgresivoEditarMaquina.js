...
$(document).ready(function () {
    ...
    $("#btnGuardar").click(function () {
        ...
        if (validar.isValid()) {
            ...
            let datasend={}
            if($("#consultaVpn").val().toLowerCase()=='true'){
                uriConsulta+="Progresivo/ConsultarModificarMaquinaProgresivoVpn"
                datasend={
                    parametros:dataForm,
                    urlPrivada:$("#urlPrivada").val()+"/servicio/ModificarMaquina?codProgresivo=" + $("#codProgresivo").val(),
                    urlPublica:$("#urlPublica").val()+"/servicio/ModificarMaquinaVpn"
                }
            }
            else{
                  //
                var url_ = $("#url_sala").val() + "/servicio/ModificarMaquina?codProgresivo=" + $("#codProgresivo").val();
                uriConsulta+="Progresivo/ConsultarModificarMaquinaProgresivo"
                datasend={
                    parametros:dataForm,
                    url:url_
                }
...
function ListarDatosMaquina() {
    ...
    if($("#consultaVpn").val().toLowerCase()=='true'){
        dataForm={
            urlPrivada:$("#urlPrivada").val()+ "/servicio/ObtenerMaquina?codProgresivo=" + codprogresivo + "&maquina=" + idmaquina,
            urlPublica:$("#urlPublica").val()+"/servicio/ObtenerMaquinaVpn"
        }
        uriConsulta+="Progresivo/ConsultarObtenerMaquinaJsonVpn"
    }
    else{
        ...
        uriConsulta+="Progresivo/ConsultarObtenerMaquinaJson"
    }
...
function ObtenerListaMarcaMaquina(MarcaId) {
    ...
    if($("#consultaVpn").val().toLowerCase()=='true'){
        dataForm={
            urlPrivada:$("#urlPrivada").val()+"/servicio/getMarcas?codProgresivo=" + $("#codProgresivo").val(),
            urlPublica:$("#urlPublica").val()+"/servicio/getMarcasVpn"
        }
        uriConsulta+="Progresivo/ConsultarListaMarcaMaquinaJsonVpn"
    }
    else{
        dataForm={
            url:url_
        }
        uriConsulta+="Progresivo/ConsultarListaMarcaMaquinaJson"
    }
    ajaxhr =$.ajax({
        type: "POST",
        url: uriConsulta,
...
function ObtenerModeloMaquinaGet(marca_id, ModeloID) {
    ...
    if($("#consultaVpn").val().toLowerCase()=='true'){
        dataForm={
            urlPrivada:$("#urlPrivada").val()+"/servicio/ListarModelosidmarca?codProgresivo=" + $("#codProgresivo").val() + "&idmarca=" + marca_id,
            urlPublica:$("#urlPublica").val()+"/servicio/ListarModelosidmarcaVpn"
        }
        uriConsulta+="Progresivo/ConsultarListaModeloMaquinaJsonVpn"
    }
    else{
        dataForm={
            url:$("#url_sala").val() + "/servicio/ListarModelosidmarca?codProgresivo=" + $("#codProgresivo").val() + "&idmarca=" + marca_id
        }
        uriConsulta+="Progresivo/ConsultarListaModeloMaquinaJson"
    }
...
function ObtenerJuegoMaquinaGet(juego) {
    $("#cboJuego").empty();
    $("#cboJuego").append('<option value="">--Seleccione--</option>');
    var url_ = $("#url_sala").val() + "/servicio/ListarMaquinas?estado=2";

    let uriConsulta = basePath;
    let dataForm = {}
    if ($("#consultaVpn").val().toLowerCase() == 'true') {
        dataForm = {
            urlPrivada: $("#urlPrivada").val() + "/servicio/getJuegos",
            urlPublica: $("#urlPublica").val() + "/servicio/getJuegosVpn"
        }
        uriConsulta += "Progresivo/ConsultarListaJuegoMaquinaJsonVpn"
    }
    else {
        dataForm = {
            url: $("#url_sala").val() + "/servicio/getJuegos"
        }
        uriConsulta += "Progresivo/ConsultarListaJuegoMaquinaJson"
    }
    ...
    ajaxhr = $.ajax({
        ...
        success: function (result) {
            var datos = result.data;
            $.each(datos, function (index, value) {
                if (juego == value.Juego) {
                    select = "selected";
                }
                else {
                    select = "";
                }
                $("#cboJuego").append('<option ' + select + ' value="' + value.Juego + '"  data-id="' + value.Juego + '"  >' + value.Juego + '</option>');
            });
        },
        error: function (request, status, error) {
            toastr.error("Error", "Mensaje Servidor");
