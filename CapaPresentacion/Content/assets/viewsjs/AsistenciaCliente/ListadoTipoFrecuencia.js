$(document).ready(function(){
    ...
    $('#btnNuevoRegistro').on('click', function (e) {
        window.location.replace(basePath + "AsistenciaCliente/RegistroTipoFrecuencia");
    });
    $(document).on('click', '.btnEditar', function (e) {
        let id = $(this).data("id");
        let url = basePath + "AsistenciaCliente/EditarTipoFrecuencia/" + id;
    ...
    $(document).on("click", "#btnExcel", function () {
        $.ajax({
            type: "POST",
            cache: false,
            url: basePath + "AsistenciaCliente/GetListadoTipoFrecuenciaExcel",
...
function ListarTipoFrecuencia(){
    let url = basePath + "AsistenciaCliente/GetListadoTipoFrecuencia";
    let data = {};
    $.ajax({
        url: url,
        type: "POST",
        ...
        success: function (response) {
            if(response.respuesta){
                if(response.data.length>0){
                    objetodatatable = $("#tableTipoFrecuencia").DataTable({
                        "bDestroy": true,
                        "bSort": false,
                        "scrollCollapse": true,
                        "scrollX": false,
