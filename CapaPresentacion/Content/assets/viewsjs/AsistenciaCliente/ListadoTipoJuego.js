$(document).ready(function () {
    ...
    $('#btnNuevoRegistro').on('click', function (e) {
        window.location.replace(basePath + "AsistenciaCliente/RegistroTipoJuego");
    });
    $(document).on('click', '.btnEditar', function (e) {
        let id = $(this).data("id");
        let url = basePath + "AsistenciaCliente/EditarTipoJuego/" + id;
    ...
    $(document).on("click", "#btnExcel", function () {
        $.ajax({
            type: "POST",
            cache: false,
            url: basePath + "AsistenciaCliente/GetListadoTipoJuegoExcel",
...
function ListarTipoJuego() {
    let url = basePath + "AsistenciaCliente/GetListadoTipoJuego";
    let data = {};
    $.ajax({
        url: url,
        type: "POST",
        ...
        success: function (response) {
            if (response.respuesta) {
                if (response.data.length > 0) {
                    objetodatatable = $("#tableTipoJuego").DataTable({
                        "bDestroy": true,
                        "bSort": false,
                        "scrollCollapse": true,
                        "scrollX": false,
